...
# Opens the file and dumps it's contents into a well formatted string
with open("source_file_2.json") as f:
    json_data = json.load(f)

# Function for getting every project that a watcher is in
def watchers():
    ...
    dic_geral = {}
    # Tests i watcher for every project
    for i in range(len(watchers_list)):
        projects_list = list()
        for j in range(len(json_data)):
            data_watcher = json_data[j]['watchers']

            # If found will add to list based on priority value
...
def managers():
    ...
    for i in range(len(managers_list)):
        ...
        # Appends manager i with list of projects hes in
        dic_geral.update({
